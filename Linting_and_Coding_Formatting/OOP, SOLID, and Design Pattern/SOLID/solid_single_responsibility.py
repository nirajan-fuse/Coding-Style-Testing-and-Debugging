"""
This module defines classes for managing a library catalog and book borrowing.
"""
...
# functionalities:
# Add books by storing each book objects (Hint: Create an empty list in constructor and store book
# objects) get book details and get all books from the list of objects.

# Lets say, we need a book borrowing process (what books are borrowed and what books are available
# for borrowing). Implement logics to integrate this requirement in the above system. Design the
# classes with a clear focus on adhering to the Single Responsibility Principle(SRP) which
# represents that "A module should be responsible to one, and only one, actor."
...
class LibraryCatalog:
    ...
    def get_all_books_detail(self):
        for book in self.books:
            print(
                f'Book_Name: {book.title}, Author: {book.author}, ISBN: {book.isbn}, Genre: {book.genre}, Availability: {"Available" if book.availability==True else "Not Available"}'
            )
